...
import com.pairtradinglab.ptltrader.trading.kernelfx.SubModelKalmanGrid;
import net.jcip.annotations.NotThreadSafe;
import org.apache.log4j.Logger;

@NotThreadSafe
public class PairTradingModelKalmanGrid extends PairTradingModel {
    private static final int UNSTABLE_PERIOD = 60;
    // settings
    private int neutrality = PairStrategy.NEUTRALITY_DOLLAR;

    // state
    private TripleBandLogic tbl;
    SubModelKalmanGrid grid;
    double lastScore;
    private double lastExitScore;

    PairTradingModelKalmanGrid(MarketRates mr1, MarketRates mr2, Logger logger) {
    ...
    String getStatusInfo() { // should never fail
        ...
        return String.format("Beta=%.4f Intercept=%.4f StdDev=%.4f LastZScore=%.4f", grid.getBeta(), grid.getAlpha(), grid.getSq(), lastScore);
    }

    @Override
    public void setupFromStrategy(PairStrategy ps) {
        tbl = new TripleBandLogic(ps.getEntryThreshold(), ps.getExitThreshold(), ps.getDowntickThreshold(), ps.getMaxEntryScore(), ps.getEntryMode());
        setNeutrality(ps.getNeutrality());
        grid = new SubModelKalmanGrid();
